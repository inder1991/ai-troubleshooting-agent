"""
Agent Registry: defines all 38 diagnostic agents, their configurations,
...
# ---------------------------------------------------------------------------
# Agent Registry: 38 agents (15 app_diagnostics + 18 cluster_diagnostics + 4 database_diagnostics + 1 assistant)
# ---------------------------------------------------------------------------
...
AGENT_REGISTRY: list[dict[str, Any]] = [
    ...
    # --- RBAC Domain Expert (added Phase 2) ---
    {
        "id": "rbac_agent",
        "name": "RBAC_AGENT",
        "workflow": "cluster_diagnostics",
        "role": "domain_expert",
        "description": "Analyzes RBAC configurations including ServiceAccounts, Roles, RoleBindings, and permission issues.",
        "icon": "admin_panel_settings",
        "level": 4,
        "llm_config": {"model": "claude-haiku-4-5-20251001", "temperature": 0.1, "context_window": 128000, "mode": "react"},
        "timeout_s": 60,
        "tools": ["list_rbac", "list_pods", "list_events"],
        "tool_health_checks": {"k8s_api": "check_k8s_connectivity"},
        "architecture_stages": ["RBAC Scan", "Binding Analysis", "LLM Analysis", "Report Build"],
    },

    # --- Diagnostic Intelligence Pipeline (6 deterministic agents) ---
    {
        "id": "signal_normalizer",
        "name": "SIGNAL_NORMALIZER",
        "workflow": "cluster_diagnostics",
        "role": "orchestrator",
        "description": "Extracts canonical signals from domain report anomalies with reliability weights. Zero LLM cost.",
        "icon": "tune",
        "level": 3,
        "llm_config": {"model": "none", "temperature": 0.0, "context_window": 0, "mode": "deterministic"},
        "timeout_s": 3,
        "tools": ["signal_extractor"],
        "tool_health_checks": {},
        "architecture_stages": ["Anomaly Parse", "Signal Extract", "Reliability Score", "Dedup"],
    },
    {
        "id": "failure_pattern_matcher",
        "name": "PATTERN_MATCHER",
        "workflow": "cluster_diagnostics",
        "role": "orchestrator",
        "description": "Matches signals against 15 known K8s failure patterns with priority-based conflict resolution. Zero LLM cost.",
        "icon": "pattern",
        "level": 3,
        "llm_config": {"model": "none", "temperature": 0.0, "context_window": 0, "mode": "deterministic"},
        "timeout_s": 5,
        ...
        "tool_health_checks": {},
        "architecture_stages": ["Signal Match", "Condition Check", "Priority Resolve", "Pattern Output"],
    },
    {
        ...
        "workflow": "cluster_diagnostics",
        "role": "orchestrator",
        "description": "Computes event recency, restart velocity, and worsening detection from K8s timestamps. Zero LLM cost.",
        "icon": "schedule",
        "level": 3,
        "llm_config": {"model": "none", "temperature": 0.0, "context_window": 0, "mode": "deterministic"},
        "timeout_s": 3,
        "tools": ["timestamp_parser"],
        "tool_health_checks": {},
        "architecture_stages": ["Timestamp Parse", "Velocity Calc", "Worsening Detect", "Flap Detect"],
    },
    {
        "id": "diagnostic_graph_builder",
        "name": "EVIDENCE_GRAPH",
        "workflow": "cluster_diagnostics",
        "role": "orchestrator",
        "description": "Builds cross-domain diagnostic evidence graph with typed edges (CAUSES, AFFECTS, DEPENDS_ON, SYMPTOM_OF). Zero LLM cost.",
        "icon": "hub",
        "level": 4,
        "llm_config": {"model": "none", "temperature": 0.0, "context_window": 0, "mode": "deterministic"},
        "timeout_s": 5,
        "tools": ["graph_builder"],
        "tool_health_checks": {},
        "architecture_stages": ["Node Create", "Edge Rules", "Topology Link", "Validate"],
    },
    {
        "id": "issue_lifecycle_classifier",
        "name": "LIFECYCLE_CLASSIFIER",
        "workflow": "cluster_diagnostics",
        "role": "orchestrator",
        "description": "Classifies issues into 9 lifecycle states (ACTIVE_DISRUPTION through ACKNOWLEDGED) with priority scoring. Zero LLM cost.",
        "icon": "category",
        "level": 4,
        "llm_config": {"model": "none", "temperature": 0.0, "context_window": 0, "mode": "deterministic"},
        "timeout_s": 5,
        "tools": ["state_classifier"],
        "tool_health_checks": {},
        "architecture_stages": ["State Classify", "Priority Score", "Blast Radius", "Issue Build"],
    },
    {
        ...
        "workflow": "cluster_diagnostics",
        "role": "orchestrator",
        "description": "Multi-hypothesis root cause engine with 3-source generation, negative evidence, logistic confidence scoring. Zero LLM cost.",
        "icon": "psychology",
        "level": 5,
        "llm_config": {"model": "none", "temperature": 0.0, "context_window": 0, "mode": "deterministic"},
        "timeout_s": 10,
        "tools": ["hypothesis_scorer"],
        "tool_health_checks": {},
        "architecture_stages": ["Pattern Seed", "Graph Traverse", "Signal Correlate", "Score + Rank"],
    },
    {
        ...
        "workflow": "cluster_diagnostics",
        "role": "validation",
        "description": "Validates remediation safety: forbidden commands, impact simulation, replica safety, confidence scoring.",
        "icon": "verified_user",
        "level": 3,
        "llm_config": {"model": "none", "temperature": 0.0, "context_window": 0, "mode": "deterministic"},
        "timeout_s": 8,
        "tools": ["command_validator", "simulator"],
        "tool_health_checks": {},
        "architecture_stages": ["Forbidden Check", "Simulate Impact", "Safety Check", "Confidence Score"],
    },

    # =========================================================================
    # DATABASE DIAGNOSTICS (4 agents)
    # =========================================================================
    {
        "id": "query_analyst",
        "name": "QUERY_ANALYST",
        "workflow": "database_diagnostics",
        "role": "analysis",
        "description": "Analyzes slow queries, execution plans, missing indexes, and query patterns using database-specific tools.",
        "icon": "query_stats",
        "level": 3,
        "llm_config": {"model": "claude-haiku-4-5-20251001", "temperature": 0.0, "context_window": 128000, "mode": "react"},
        "timeout_s": 60,
        "tools": ["run_explain", "get_slow_queries", "get_table_stats"],
        "tool_health_checks": {},
        "architecture_stages": ["Query Fetch", "EXPLAIN Analysis", "Index Check", "Finding Report"],
    },
    {
        "id": "health_analyst",
        "name": "HEALTH_ANALYST",
        "workflow": "database_diagnostics",
        "role": "analysis",
        "description": "Monitors database health metrics: connections, locks, replication lag, vacuum status, and wait events.",
        "icon": "health_and_safety",
        "level": 3,
        "llm_config": {"model": "claude-haiku-4-5-20251001", "temperature": 0.0, "context_window": 128000, "mode": "react"},
        "timeout_s": 60,
        "tools": ["get_connections", "get_locks", "get_replication_status", "get_wait_events"],
        "tool_health_checks": {},
        "architecture_stages": ["Health Scan", "Lock Analysis", "Replication Check", "Finding Report"],
    },
    {
        "id": "schema_analyst",
        "name": "SCHEMA_ANALYST",
        "workflow": "database_diagnostics",
        "role": "analysis",
        "description": "Analyzes schema structure, table bloat, unused indexes, and schema drift issues.",
        "icon": "schema",
        "level": 3,
        "llm_config": {"model": "claude-haiku-4-5-20251001", "temperature": 0.0, "context_window": 128000, "mode": "react"},
        "timeout_s": 60,
        "tools": ["get_schema", "get_index_usage", "get_table_bloat"],
        "tool_health_checks": {},
        "architecture_stages": ["Schema Scan", "Bloat Check", "Index Analysis", "Finding Report"],
    },
    {
        "id": "db_synthesizer",
        "name": "DB_SYNTHESIZER",
        "workflow": "database_diagnostics",
        "role": "orchestrator",
        "description": "Synthesizes findings from all database analysts into root cause analysis with fix recommendations and SQL.",
        "icon": "merge",
        "level": 5,
        "llm_config": {"model": "claude-sonnet-4-20250514", "temperature": 0.1, "context_window": 200000, "mode": "autonomous"},
        "timeout_s": 60,
        "tools": ["llm_synthesis"],
        "tool_health_checks": {},
        "architecture_stages": ["Finding Merge", "Root Cause Analysis", "Fix Generation", "SQL Validation"],
    },
    ...
    # =========================================================================
    {
        ...
        "workflow": "assistant",
        "role": "orchestrator",
        "description": "Natural language command interface for SRE operations. Handles queries, starts investigations, navigates the platform.",
        "icon": "smart_toy",
        "level": 5,
        "llm_config": {"model": "claude-sonnet-4-20250514", "temperature": 0.0, "context_window": 200000, "mode": "autonomous"},
        "timeout_s": 30,
        "tools": ["list_sessions", "get_session_detail", "start_investigation", "navigate_to"],
        "tool_health_checks": {},
        "architecture_stages": ["Intent Parse", "Tool Select", "Execute", "Format Response"],
    },
]
