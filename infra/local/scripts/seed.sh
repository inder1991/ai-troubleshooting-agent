...
if ! docker compose -f infra/local/compose.dev.yml ps postgres --status running --quiet | grep -q .; then
...
docker compose -f infra/local/compose.dev.yml exec -T backend-web \
