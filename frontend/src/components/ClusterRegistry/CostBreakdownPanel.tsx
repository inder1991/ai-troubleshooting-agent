  ...
  return (
    <div className="bg-[#1e1b15] border border-[#3d3528]/50 rounded-lg p-5">
      ...
      <div className="grid grid-cols-3 gap-4 mb-5">
        ...
        <div className="bg-[#13110d] rounded-lg p-3 text-center">
          <div className="text-[10px] text-slate-500 mb-1">Projected</div>
        ...
        <div className="bg-[#13110d] rounded-lg p-3 text-center">
          ...
          <div className="text-[10px] text-[#e09f3e]/70">{savingsPct}% reduction</div>
        </div>
      </div>

      {/* Idle capacity bars */}
      <div className="mb-5">
        <div className="text-[10px] uppercase tracking-wider text-slate-500 font-medium mb-3">Idle Capacity</div>
        <div className="space-y-3">
          {/* CPU bar */}
          <div>
            <div className="flex items-center justify-between mb-1">
              <span className="text-[11px] text-slate-400">CPU</span>
              <span className={`text-[11px] font-medium ${cost.idle_cpu_pct > 40 ? 'text-[#e09f3e]' : 'text-slate-300'}`}>
          ...
          <div>
            <div className="flex items-center justify-between mb-1">
              ...
              <span className={`text-[11px] font-medium ${cost.idle_memory_pct > 40 ? 'text-[#e09f3e]' : 'text-slate-300'}`}>
      ...
      {cost.instance_breakdown.length > 0 && (
        <div className="mb-5">
          <div className="text-[10px] uppercase tracking-wider text-slate-500 font-medium mb-2">Instance Types</div>
      ...
      {cost.namespace_costs.length > 0 && (
        <div>
          <div className="text-[10px] uppercase tracking-wider text-slate-500 font-medium mb-2">Top Namespaces by Cost</div>
          <div className="space-y-1.5">
            {cost.namespace_costs.slice(0, 8).map((ns, i) => (
              <div key={i} className="flex items-center gap-2">
                <span className="text-[11px] text-slate-400 font-mono flex-1 truncate">{ns.namespace}</span>
                <span className="text-[10px] text-slate-500">{ns.pod_count} pods</span>
                <span className="text-[11px] text-slate-300 font-medium w-16 text-right">
                  ${ns.estimated_cost.toFixed(0)}
                </span>
                <span className="text-[10px] text-slate-500 w-10 text-right">{ns.cost_pct.toFixed(1)}%</span>
              </div>
            ))}
          </div>
        </div>
