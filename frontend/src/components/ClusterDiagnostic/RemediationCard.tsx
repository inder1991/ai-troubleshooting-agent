    ...
    <div className="bg-wr-surface border border-wr-border rounded-lg p-4 shadow-lg">
        ...
        <div className="mb-3">
          ...
          <div className="bg-black/40 rounded p-2 font-mono text-body-xs text-blue-400 mt-1 border border-wr-border/30 flex items-center justify-between gap-2">
            ...
            <button
              ...
              className="shrink-0 text-body-xs px-2 py-0.5 rounded border border-blue-500/30 text-blue-400 hover:bg-blue-500/10 transition-colors"
            >
              Run
            </button>
          </div>
      ...
      {primaryStep.validation_errors && primaryStep.validation_errors.length > 0 && (
        <div className="mb-3 px-2 py-1.5 rounded border border-red-500/20 bg-red-500/5">
          <span className="text-body-xs uppercase font-semibold text-red-400 tracking-wider">Validation Errors</span>
          {primaryStep.validation_errors.map((err, i) => (
            <div key={i} className="text-body-xs text-red-300 mt-0.5">• {err}</div>
          ))}
        </div>
      )}
      ...
      {primaryStep.validation?.blocked && (
        <div className="mb-3 px-2 py-1.5 rounded border border-red-500/20 bg-red-500/5">
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-body-xs text-red-400 font-semibold">Blocked</span>
          </div>
          {primaryStep.validation.block_reason && (
            <p className="text-body-xs text-red-300/70 mt-1">{primaryStep.validation.block_reason}</p>
          )}
        </div>
      )}
      ...
      {primaryStep.validation?.requires_confirmation && !primaryStep.validation?.blocked && primaryStep.command && (
        <div className="mb-2 flex items-center gap-1.5 px-2 py-1 rounded bg-amber-500/5 border border-amber-500/20">
          ...
          <span className="text-body-xs text-amber-400">This action requires manual confirmation before execution</span>
        </div>
      )}
      ...
      {primaryStep.command && !primaryStep.validation?.blocked && (
        ...
        >
          ...
          <div className="relative w-8 h-8 z-10">
            ...
            <div className={`absolute inset-0 flex items-center justify-center text-chrome font-mono font-bold transition-colors ${
              holdingIndex === 0 ? 'text-white' : 'text-wr-accent'
            }`}>
              HOLD
            </div>
        ...
        <div className="flex gap-2 mt-2">
          {primaryStep.verify && (
            <button
              ...
              className="flex-1 text-body-xs px-3 py-1.5 rounded border border-emerald-500/30 text-emerald-400 hover:bg-emerald-500/10 transition-colors font-semibold uppercase tracking-wider"
            >
              Verify
            </button>
          )}
            ...
            <div key={i} className="text-xs text-slate-400">
              ...
              {step.pre_check && (
                <div className="flex items-center gap-1 mt-1">
                  <code className="text-body-xs text-blue-400/70 font-mono">pre: {step.pre_check}</code>
                </div>
              )}
              {step.command && (
                <code className="text-body-xs text-wr-accent block mt-1 font-mono">$ {step.command}</code>
              )}
              {step.rollback && (
                <code className="text-body-xs text-red-400/50 block mt-0.5 font-mono">rollback: {step.rollback}</code>
              )}
            </div>
          ))}
        </div>
