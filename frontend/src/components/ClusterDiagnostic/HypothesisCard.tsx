  ...
  if (!hasHypotheses && !primaryChain) {
    return (
      <div className="border border-wr-border-subtle rounded bg-wr-inset p-4">
        <h3 className="text-body-xs uppercase font-bold tracking-widest text-slate-400 mb-1 flex items-center gap-2">
          <span className="material-symbols-outlined text-sm">search</span>
          Root Cause Analysis
        </h3>
        <p className="text-body-xs text-slate-500 animate-pulse">Correlating events...</p>
      </div>
    );
  }
    ...
    return (
      <div className="border border-wr-border-subtle rounded bg-wr-inset">
        <div className="px-4 pt-4 pb-2">
          <h3 className="text-body-xs uppercase font-bold tracking-widest text-slate-400 mb-3 flex items-center gap-2">
              ...
              return (
                ...
                >
                  <div className="flex items-start gap-2">
                    <span className="text-body-xs font-mono text-slate-500 mt-0.5 shrink-0">#{idx + 1}</span>
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center flex-wrap gap-x-3 gap-y-0.5 mt-1">
                        ...
                        <span className="text-body-xs text-slate-500 capitalize">
                          Source: {h.source}
                        </span>
                      </div>
                    </div>
        ...
        {effects.length > 0 && (
          <div className="border-t border-wr-border-subtle">
            <button
              ...
              className="w-full px-4 py-2 flex items-center gap-2 text-body-xs text-slate-400 hover:text-slate-400 transition-colors"
            >
              ...
              <div className="px-4 pb-3 space-y-1">
                {effects.map((eff, i) => (
                  <div key={i} className="flex items-start gap-2 pl-4">
                    <span className="text-body-xs text-slate-500 font-mono mt-px shrink-0">{i + 1}.</span>
                    <span className="text-body-xs text-slate-400">{eff}</span>
                  </div>
                ))}
              </div>
            )}
    ...
    >
      ...
      <div className="flex gap-4">
        <div className="p-2 bg-wr-bg/60 border border-wr-border-subtle rounded">
          <div className="text-chrome uppercase text-slate-400">Confidence</div>
          <div className="text-base font-mono" style={{ color: badgeColor }}>{Math.round(chain.confidence * 100)}%</div>
        </div>
        <div className="p-2 bg-wr-bg/60 border border-wr-border-subtle rounded">
          <div className="text-chrome uppercase text-slate-400">Cascading Effects</div>
      ...
      {chain.cascading_effects.length > 0 && (
        <div className="border-t border-wr-border-subtle mt-3 pt-2">
          <button
            ...
            aria-label={expanded ? 'Collapse effects' : 'Show cascading effects'}
            aria-expanded={expanded}
            className="flex items-center gap-1 text-body-xs text-slate-400 hover:text-slate-400 transition-colors"
          >
            <span className="material-symbols-outlined text-xs" style={{ transform: expanded ? 'rotate(90deg)' : 'none', transition: 'transform 150ms' }}>
              chevron_right
            </span>
          ...
          {expanded && (
            <div className="mt-2 space-y-1">
              {chain.cascading_effects.map((eff, i) => (
                <div key={i} className="flex items-start gap-2 pl-4">
                  <span className="text-body-xs text-slate-500 font-mono mt-px shrink-0">{eff.order}.</span>
                  <span className="text-body-xs text-slate-400">{eff.description}</span>
                </div>
              ))}
            </div>
          )}
