  ...
  return (
    <div className="absolute top-full right-0 mt-1 z-50 bg-wr-inset border border-wr-border-subtle rounded-lg shadow-xl w-[min(480px,calc(100vw-2rem))] p-3">
      <div className="flex items-center justify-between mb-2">
        <span className="text-body-xs font-semibold uppercase tracking-wider text-slate-400">LLM Cost Breakdown</span>
        <button onClick={onClose} aria-label="Close cost breakdown" onKeyDown={(e) => { if (e.key === 'Escape') onClose(); }} className="text-slate-500 hover:text-slate-300">
          <span className="material-symbols-outlined text-[14px]">close</span>
        </button>
      </div>

      {error && (
        <p className="text-body-xs text-red-400 py-4 text-center">Failed to load cost data</p>
      )}

      {!error && <table className="w-full text-body-xs font-mono">
        <thead>
          <tr className="text-slate-400 border-b border-wr-border-subtle">
        ...
        <tbody>
          {agentList.map(agent => (
            <tr key={agent.agent_name} className="text-slate-300 border-b border-wr-border-subtle/50 hover:bg-wr-bg">
              ...
              <td className="text-right py-1.5 px-1 text-slate-400">{agent.tokens}</td>
              ...
              <td className="text-right py-1.5 pl-1">
                ...
                <span className="ml-1 text-slate-500">{agent.source === 'heuristic' ? 'Heur' : 'LLM'}</span>
              </td>
            </tr>
          ))}
        </tbody>
        <tfoot>
          <tr className="text-slate-400 border-t border-wr-border-subtle font-semibold">
            ...
            <td className="text-right py-1.5 px-1 text-slate-400">{Math.round(totals.inputTokens/1000)}K/{Math.round(totals.outputTokens/1000)}K</td>
