...
const DriftEventsList: React.FC<Props> = ({ drifts }) => {
  ...
  return (
    <div className="rounded-lg border" style={{ backgroundColor: '#0a1a1e', borderColor: '#224349' }}>
      <div className="flex items-center justify-between px-3 py-2 border-b" style={{ borderColor: '#224349' }}>
        ...
        <div className="flex gap-2 text-[10px] font-mono">
          ...
          {infoCount > 0 && <span style={{ color: '#07b6d5' }}>{infoCount} info</span>}
        </div>
      </div>
      <div className="max-h-48 overflow-y-auto">
        {drifts.map((d) => (
          <div
            key={d.id}
            className="px-3 py-1.5 border-b cursor-pointer hover:bg-[#162a2e]"
          ...
          >
            <div className="flex items-center gap-2 text-xs">
              ...
              />
              <span style={{ color: '#e2e8f0' }}>{d.entity_type}</span>
              <span style={{ color: '#64748b' }}>{d.drift_type}</span>
              <span className="font-mono" style={{ color: '#94a3b8' }}>{d.field}</span>
            </div>
            {expanded === d.id && (
