...
const IntegrationStatusBadge: React.FC<IntegrationStatusBadgeProps> = ({ integration, name }) => {
  if (integration.configured && integration.has_credentials && integration.status === 'connected') {
    return (
      <span className="inline-flex items-center gap-1 text-body-xs px-1.5 py-0.5 rounded-full bg-green-500/10 text-green-400 border border-green-500/20">
  ...
  if (integration.status === 'mock_available') {
    return (
      <span className="inline-flex items-center gap-1 text-body-xs px-1.5 py-0.5 rounded-full bg-violet-500/10 text-violet-400 border border-violet-500/20">
