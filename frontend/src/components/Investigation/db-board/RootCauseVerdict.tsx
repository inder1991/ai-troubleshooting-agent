    ...
    >
      <div className="flex items-center justify-between mb-1.5">
        <div className="flex items-center gap-2">
          ...
          <span className={`text-[9px] sm:text-[10px] font-bold px-1.5 py-0.5 rounded border ${badge}`}>
      ...
      {recommendation && (
        <p className="text-[11px] text-slate-300 italic border-t border-duck-border/30 pt-2 mt-2">
      ...
      {contributingPanels && contributingPanels.length > 0 && (
        <div className="flex items-center gap-1.5 mt-2">
          ...
          {contributingPanels.map((p, i) => (
            <span key={`${p}-${i}`} className="text-[9px] px-1.5 py-0.5 rounded bg-duck-accent/20 text-duck-accent">
              {p}
            </span>
          ))}
        </div>
      ...
      {evidenceWeights && Object.keys(evidenceWeights).length > 0 && (
        <div className="mt-2 space-y-1">
          <span className="text-[9px] text-slate-500">Evidence weights:</span>
          {Object.entries(evidenceWeights).map(([id, { weight, reason }]) => (
            <div key={id} className="flex items-center gap-2">
              ...
              <span className="text-[9px] text-slate-400 truncate">{reason}</span>
            </div>
          ))}
        </div>
      )}
