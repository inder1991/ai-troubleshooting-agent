  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2 px-4">
        <span className="material-symbols-outlined text-[16px] text-amber-500">account_tree</span>
        <span className="text-body-xs font-black text-slate-300 tracking-[0.1em] uppercase">Causal Forest</span>
        <span className="text-body-xs text-slate-400 font-mono">{forest.length} root cause{forest.length !== 1 ? 's' : ''}</span>
      </div>
      {forest.map(tree => (
