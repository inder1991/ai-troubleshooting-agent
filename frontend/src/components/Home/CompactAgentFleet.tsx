  ...
  if (isLoading) {
    return (
      <div className="flex items-center gap-2">
        ...
        <span className="text-[10px] text-slate-400">Loading agents...</span>
      </div>
    );
  }
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-1.5">
        <h3 className="text-[10px] font-display font-bold text-slate-400">Agent Fleet</h3>
      ...
      {activeCount === 0 ? (
        <div className="flex items-center gap-2 py-1">
          ...
          <span className="text-[10px] text-slate-400">{totalCount} agents on standby</span>
        </div>
      ) : (
        <div className="flex flex-wrap gap-1.5">
          {activeNames.map((name) => (
            <span key={name} className="flex items-center gap-1 text-[10px] px-1.5 py-0.5 rounded bg-duck-accent/10 text-duck-accent border border-duck-accent/20">
              <span className="w-1 h-1 rounded-full bg-duck-accent animate-pulse" aria-hidden="true" />
              {name}
            </span>
          ))}
          {activeCount > 4 && (
            <span className="text-[10px] text-slate-400">+{activeCount - 4} more</span>
          )}
        </div>
      )}
    </div>
