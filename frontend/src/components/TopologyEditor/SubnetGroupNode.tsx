...
const SubnetGroupNode: React.FC<NodeProps<SubnetGroupData>> = ({ data, selected }) => {
  return (
    ...
    >
      {/* 4 handles — top+left=target, bottom+right=source */}
      ...
      <NodeResizeControl
        minWidth={200}
        minHeight={150}
        style={{ background: '#07b6d5', width: '8px', height: '8px', borderRadius: '2px', pointerEvents: 'auto' }}
      />

      {/* CIDR label at top */}
      <div
        className="container-interactive absolute top-0 left-3 -translate-y-1/2 px-2 py-0.5 rounded text-[10px] font-mono font-semibold"
      ...
      {/* Subnet name */}
      <div
        className="container-interactive absolute bottom-2 left-3 text-[10px] font-mono"
        style={{ color: '#64748b' }}
      >
        {data.label}
      </div>
